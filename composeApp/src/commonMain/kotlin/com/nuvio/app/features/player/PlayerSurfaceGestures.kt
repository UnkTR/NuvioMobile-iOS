...
private const val PlayerHorizontalSeekLiveThrottleMs = 120L
...
internal fun Modifier.playerSurfaceDragGestures(
    ...
    revealLockedOverlayState: State<() -> Unit>,
    beginHorizontalSeekState: State<() -> Unit>,
    liveHorizontalSeekState: State<(Long) -> Unit>,
    ...
    pointerInput(gestureController, layoutSize, sideGestureSystemEdgeExclusionPx) {
        awaitEachGesture {
            ...
            var lastLiveHorizontalSeekMark: TimeSource.Monotonic.ValueTimeMark? = null
            ...
            while (true) {
                ...
                if (gestureMode == null) {
                    ...
                    gestureMode = when {
                        horizontalDominant -> {
                            deactivateHoldToSpeedState.value()
                            beginHorizontalSeekState.value()
                            lastLiveHorizontalSeekMark = TimeSource.Monotonic.markNow()
                ...
                when (gestureMode) {
                    PlayerGestureMode.HorizontalSeek -> {
                        ...
                        val elapsedSinceLastLiveSeekMs =
                            lastLiveHorizontalSeekMark?.elapsedNow()?.inWholeMilliseconds
                                ?: Long.MAX_VALUE
                        if (elapsedSinceLastLiveSeekMs >= PlayerHorizontalSeekLiveThrottleMs) {
                            liveHorizontalSeekState.value(horizontalSeekPreviewMs)
                            lastLiveHorizontalSeekMark = TimeSource.Monotonic.markNow()
                        }
                    }
